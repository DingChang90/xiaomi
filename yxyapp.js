...
function login(timeout = 0) {
  return new Promise((resolve) => {
    ...
    let sign = MD5_Encrypt(`password=${mima}&timestamp=${time}&username=${user}`)
    let url = {
      url: `${host}/api/v3/login/?timestamp=${time}`,
      headers: {
...
function doSubmit(timeout = 0) {
  return new Promise((resolve) => {
    let targets = ['1010', '1020', '1050', '1080', '1090', '1100', '1230', '1140', '1160']
...
function doGuess(timu, dati) {
  return new Promise((resolve) => {
    let time = Math.round(new Date().getTime() / 1000).toString()
    let sign = MD5_Encrypt(`answer=${dati}&guess_id=${timu}&timestamp=${time}&token=${token}`)
    let url = {
      url: `${host}/api/v3/doGuess/?timestamp=${time}`,
      headers: {
        'Content-Type': 'application/json',
        'user-agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_0_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Html5Plus/1.0 (Immersed/20)',
        'sign': sign
...
function integral_record(num) {
  return new Promise((resolve) => {
    let time = Math.round(new Date().getTime() / 1000).toString()
    let sign = MD5_Encrypt(`id=${num}&timestamp=${time}&token=${token}`)
    let url = {
      url: `${host}/api/v3/integral_record/?timestamp=${time}`,
      headers: {
        'Content-Type': 'application/json',
        'user-agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_0_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Html5Plus/1.0 (Immersed/20)',
        'sign': sign
